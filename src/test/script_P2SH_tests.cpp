// Copyright (c) 2012-2013 The Bitcredit Core developers
...
BOOST_AUTO_TEST_CASE(sign)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        txFrom.vout[i].nValue = CREDIT;
        txFrom.vout[i+4].scriptPubKey = standardScripts[i];
        txFrom.vout[i+4].nValue = CREDIT;
    }
...
{
    LOCK(cs_main);
    CCoinsView creditsDummy;
    CCoinsViewCache credits(&creditsDummy);
    CBasicKeyStore keystore;
    CKey key[6];
    vector<CPubKey> keys;
    for (int i = 0; i < 6; i++)
    ...
    credits.ModifyCoins(txFrom.GetHash())->FromTx(txFrom, 0);
    ...
    BOOST_CHECK(::AreInputsStandard(txTo, credits));
    // 22 P2SH sigops for all inputs (1 for vin[0], 6 for vin[3], 15 for vin[4]
    BOOST_CHECK_EQUAL(GetP2SHSigOpCount(txTo, credits), 22U);
    ...
    BOOST_CHECK(!::AreInputsStandard(txToNonStd1, credits));
    BOOST_CHECK_EQUAL(GetP2SHSigOpCount(txToNonStd1, credits), 16U);
    ...
    BOOST_CHECK(!::AreInputsStandard(txToNonStd2, credits));
    BOOST_CHECK_EQUAL(GetP2SHSigOpCount(txToNonStd2, credits), 20U);
}
