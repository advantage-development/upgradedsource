 ...
 *
 *  - version = 1
 *  - code = 9 (creditbase, neither vout[0] or vout[1] are unspent, 2 (1, +1 because both bit 2 and bit 4 are unset) non-zero bitvector bytes follow)
...
{
public:
    //! whether transaction is a creditbase
    bool fCoinBase;
    bool fCoinStake;
    ...
    void Unserialize(Stream& s, int nType, int nVersion)
    {
        ...
        fCoinBase = nCode & 1;         //0001 - means creditbase
...
struct CCoinsCacheEntry {
    CCoins credits; // The actual cached data.
    unsigned char flags;
    ...
    CCoinsCacheEntry() : credits(), flags(0) {}
};
...
public:
    ...
    virtual bool GetCoins(const uint256& txid, CCoins& credits) const;
...
public:
    CCoins* operator->() { return &it->second.credits; }
    CCoins& operator*() { return it->second.credits; }
    ~CCoinsModifier();
