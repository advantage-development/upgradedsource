 ...
 * We are ~100 times smaller then bitcredit now (2015-06-23), set minTxFee 10 times higher
...
CAmount CWallet::GetBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            const CWalletTx* pcredit = &(*it).second;
            if (pcredit->IsTrusted())
                nTotal += pcredit->GetAvailableCredit();
        }
    }
...
CAmount CWallet::GetAnonymizableBalance() const
{
    ...
    CAmount nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            if (pcredit->IsTrusted())
                nTotal += pcredit->GetAnonymizableCredit();
        }
    }
...
CAmount CWallet::GetAnonymizedBalance() const
{
    ...
    CAmount nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            if (pcredit->IsTrusted())
                nTotal += pcredit->GetAnonymizedCredit();
        }
    }
...
double CWallet::GetAverageAnonymizedRounds() const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcredit->vout.size(); i++) {
                CTxIn vin = CTxIn(hash, i);

                if (IsSpent(hash, i) || IsMine(pcredit->vout[i]) != ISMINE_SPENDABLE || !IsDenominated(vin)) continue;
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcredit->vout.size(); i++) {
                ...
                if (IsSpent(hash, i) || IsMine(pcredit->vout[i]) != ISMINE_SPENDABLE || !IsDenominated(vin)) continue;
                if (pcredit->GetDepthInMainChain() < 0) continue;

                int rounds = GetInputObfuscationRounds(vin);
                nTotal += pcredit->vout[i].nValue * rounds / nObfuscationRounds;
            }
        }
    }
...
CAmount CWallet::GetDenominatedBalance(bool unconfirmed) const
{
    ...
    CAmount nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            nTotal += pcredit->GetDenominatedCredit(unconfirmed);
        }
    }
...
CAmount CWallet::GetUnconfirmedBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            const CWalletTx* pcredit = &(*it).second;
            if (!IsFinalTx(*pcredit) || (!pcredit->IsTrusted() && pcredit->GetDepthInMainChain() == 0))
                nTotal += pcredit->GetAvailableCredit();
        }
    }
    return nTotal;
...
CAmount CWallet::GetImmatureBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            const CWalletTx* pcredit = &(*it).second;
            nTotal += pcredit->GetImmatureCredit();
        }
    }
    return nTotal;
}
...
CAmount CWallet::GetWatchOnlyBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            const CWalletTx* pcredit = &(*it).second;
            if (pcredit->IsTrusted())
                nTotal += pcredit->GetAvailableWatchOnlyCredit();
        }
    }
...
CAmount CWallet::GetImmatureWatchOnlyBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            const CWalletTx* pcredit = &(*it).second;
            nTotal += pcredit->GetImmatureWatchOnlyCredit();
        }
    }
    return nTotal;
}
 ...
 */
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl* creditControl, bool fIncludeZeroValue, AvailableCoinsType nCoinType, bool fUseIX) const
{
    vCoins.clear();

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            if (!CheckFinalTx(*pcredit))
                continue;

            if (fOnlyConfirmed && !pcredit->IsTrusted())
                continue;

            if ((pcredit->IsCoinBase() || pcredit->IsCoinStake()) && pcredit->GetBlocksToMaturity() > 0)
                continue;

            int nDepth = pcredit->GetDepthInMainChain(false);
            // do not use IX for inputs that have less then 6 blockchain confirmations
            if (fUseIX && nDepth < 6)
                continue;

            // We should not consider credits which aren't at least in our mempool
            // It's possible for these to be conflicted via ancestors which we may never be able to detect
            if (nDepth == 0 && !pcredit->InMempool())
                continue;

            for (unsigned int i = 0; i < pcredit->vout.size(); i++) {
                bool found = false;
                if (nCoinType == ONLY_DENOMINATED) {
                    found = IsDenominatedAmount(pcredit->vout[i].nValue);
                } else if (nCoinType == ONLY_NOT10000IFMN) {
                    found = !(fMasterNode && pcredit->vout[i].nValue == Params().MasternodeCollateralLimit() * CREDIT);
                } else if (nCoinType == ONLY_NONDENOMINATED_NOT10000IFMN) {
                    if (IsCollateralAmount(pcredit->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcredit->vout[i].nValue);
                    if (found && fMasterNode) found = pcredit->vout[i].nValue != Params().MasternodeCollateralLimit() * CREDIT; // do not use Hot MN funds
                } else if (nCoinType == ONLY_10000) {
                    found = pcredit->vout[i].nValue == Params().MasternodeCollateralLimit() * CREDIT;
                } else {
                    found = true;
                }
                if (!found) continue;

                isminetype mine = IsMine(pcredit->vout[i]);
                if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO &&
                    (!IsLockedCoin((*it).first, i) || nCoinType == ONLY_10000) &&
                    (pcredit->vout[i].nValue > 0 || fIncludeZeroValue) &&
                    (!creditControl || !creditControl->HasSelected() || creditControl->fAllowOtherInputs || creditControl->IsSelected((*it).first, i)))
                    vCoins.push_back(COutput(pcredit, i, nDepth,
                        ((mine & ISMINE_SPENDABLE) != ISMINE_NO) ||
                            (creditControl && creditControl->fAllowWatchOnly && (mine & ISMINE_WATCH_SOLVABLE) != ISMINE_NO)));
            }
        }
    }
}

map<CBitcreditAddress, vector<COutput> > CWallet::AvailableCoinsByAddress(bool fConfirmed, CAmount maxCoinValue)
{
    vector<COutput> vCoins;
    AvailableCoins(vCoins, fConfirmed);

    map<CBitcreditAddress, vector<COutput> > mapCoins;
    BOOST_FOREACH (COutput out, vCoins) {
        ...
        mapCoins[CBitcreditAddress(address)].push_back(out);
    }
...
{
    const CWalletTx* pcredit1 = out1.tx;
    const CWalletTx* pcredit2 = out2.tx;

    bool found1 = false;
    bool found2 = false;
    BOOST_FOREACH (CAmount d, obfuScationDenominations) // loop through predefined denoms
    {
        if (pcredit1->vout[out1.i].nValue == d) found1 = true;
        if (pcredit2->vout[out2.i].nValue == d) found2 = true;
    }
...
bool CWallet::SelectStakeCoins(std::set<std::pair<const CWalletTx*, unsigned int> >& setCoins, CAmount nTargetAmount) const
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (out.nDepth < (out.tx->IsCoinStake() ? Params().CREDITBASE_MATURITY() : 10))
            continue;
...
bool CWallet::SelectCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet) const
{
    ...
    pair<CAmount, pair<const CWalletTx*, unsigned int> > creditLowestLarger;
    creditLowestLarger.first = std::numeric_limits<CAmount>::max();
    creditLowestLarger.second.first = NULL;
    ...
    // try to find nondenom first to prevent unneeded spending of mixed credits
    for (unsigned int tryDenom = 0; tryDenom < 2; tryDenom++) {
        if (fDebug) LogPrint("selectcredits", "tryDenom: %d\n", tryDenom);
        vValue.clear();
        nTotalLower = 0;
        BOOST_FOREACH (const COutput& output, vCoins) {
            ...
            const CWalletTx* pcredit = output.tx;

            //            if (fDebug) LogPrint("selectcredits", "value %s confirms %d\n", FormatMoney(pcredit->vout[output.i].nValue), output.nDepth);
            if (output.nDepth < (pcredit->IsFromMe(ISMINE_ALL) ? nConfMine : nConfTheirs))
                continue;

            int i = output.i;
            CAmount n = pcredit->vout[i].nValue;
            if (tryDenom == 0 && IsDenominatedAmount(n)) continue; // we don't want denom values on first run

            pair<CAmount, pair<const CWalletTx*, unsigned int> > credit = make_pair(n, make_pair(pcredit, i));

            if (n == nTargetValue) {
                setCoinsRet.insert(credit.second);
                nValueRet += credit.first;
                return true;
            } else if (n < nTargetValue + CENT) {
                vValue.push_back(credit);
                nTotalLower += n;
            } else if (n < creditLowestLarger.first) {
                creditLowestLarger = credit;
            }
        }
        ...
        if (nTotalLower < nTargetValue) {
            if (creditLowestLarger.second.first == NULL) // there is no input larger than nTargetValue
            {
                if (tryDenom == 0)
            ...
            setCoinsRet.insert(creditLowestLarger.second);
            nValueRet += creditLowestLarger.first;
            return true;
        }
    ...
    // If we have a bigger credit and (either the stochastic approximation didn't find a good solution,
    //                                   or the next bigger credit is closer), return the bigger credit
    if (creditLowestLarger.second.first &&
        ((nBest != nTargetValue && nBest < nTargetValue + CENT) || creditLowestLarger.first <= nBest)) {
        setCoinsRet.insert(creditLowestLarger.second);
        nValueRet += creditLowestLarger.first;
    } else {
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* creditControl, AvailableCoinsType credit_type, bool useIX) const
{
    // Note: this function should never be used for "always free" tx types like dstx

    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, creditControl, false, credit_type, useIX);

    // credit control -> return all selected outputs (we want all selected to go into the transaction for sure)
    if (creditControl && creditControl->HasSelected()) {
        BOOST_FOREACH (const COutput& out, vCoins) {
            if (!out.fSpendable)
                continue;

            if (credit_type == ONLY_DENOMINATED) {
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 USDA
    if (credit_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * CREDIT) + 100 //round the amount up to .1 USDA over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        //if(out.tx->vout[out.i].nValue == 10000*CREDIT) continue;
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            bool fAccepted = false;

            // Function returns as follows:
            //
            // bit 0 - 10000 USDA+1 ( bit on if present )
            // bit 1 - 1000 USDA+1
            // bit 2 - 100 USDA+1
            // bit 3 - 10 USDA+1
            // bit 4 - 1 USDA+1
            ...
            if (fFound10000 && fFound1000 && fFound100 && fFound10 && fFound1 && fFoundDot1) { //if fulfilled
                ...
                if ((nDenom & (1 << 0)) && out.tx->vout[out.i].nValue == ((10000 * CREDIT) + 10000000)) {
                    fAccepted = true;
                } else if ((nDenom & (1 << 1)) && out.tx->vout[out.i].nValue == ((1000 * CREDIT) + 1000000)) {
                    fAccepted = true;
                } else if ((nDenom & (1 << 2)) && out.tx->vout[out.i].nValue == ((100 * CREDIT) + 100000)) {
                    fAccepted = true;
                } else if ((nDenom & (1 << 3)) && out.tx->vout[out.i].nValue == ((10 * CREDIT) + 10000)) {
                    fAccepted = true;
                } else if ((nDenom & (1 << 4)) && out.tx->vout[out.i].nValue == ((1 * CREDIT) + 1000)) {
                    fAccepted = true;
                } else if ((nDenom & (1 << 5)) && out.tx->vout[out.i].nValue == ((.1 * CREDIT) + 100)) {
                    fAccepted = true;
                }
            } else {
...
{
    CCoinControl* creditControl = NULL;

    setCoinsRet.clear();
    nValueRet = 0;

    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, creditControl, nObfuscationRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT10000IFMN : ONLY_DENOMINATED);
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (fMasterNode && out.tx->vout[out.i].nValue == Params().MasternodeCollateralLimit() * CREDIT) continue; //masternode input
...
bool CWallet::SelectCoinsCollateral(std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet) const
{
    ...
    //LogPrintf(" selecting credits for collateral\n");
    AvailableCoins(vCoins);

    //LogPrintf("found credits %d\n", (int)vCoins.size());
    ...
    CAmount nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            if (pcredit->IsTrusted()) {
                ...
                for (unsigned int i = 0; i < pcredit->vout.size(); i++) {
                    COutput out = COutput(pcredit, i, nDepth, true);
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);

                    if (out.tx->vout[out.i].nValue != nInputAmount) continue;
                    if (!IsDenominatedAmount(pcredit->vout[i].nValue)) continue;
                    if (IsSpent(out.tx->GetHash(), i) || IsMine(pcredit->vout[i]) != ISMINE_SPENDABLE || !IsDenominated(vin)) continue;
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    CCoinControl* creditControl = NULL;
    bool success = CreateTransaction(vecSend, tx, reservekey, nFeeRet, strFail, creditControl, ALL_CREDITS, useIX, (CAmount)0);
    if (!success) {
...
bool CWallet::CreateTransaction(const vector<pair<CScript, CAmount> >& vecSend,
    ...
    const CCoinControl* creditControl,
    AvailableCoinsType credit_type,
    bool useIX,
    CAmount nFeePay)
{
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                // vouts to the payees
                if (creditControl && !creditControl->fSplitBlock) {
                    BOOST_FOREACH (const PAIRTYPE(CScript, CAmount) & s, vecSend) {
                ...
                } else //UTXO Splitter Transaction
                {
                    ...
                    if (creditControl)
                        nSplitBlock = creditControl->nSplitBlock;
                    else
                        nSplitBlock = 1;
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, creditControl, credit_type, useIX)) {
                    if (credit_type == ALL_CREDITS) {
                        strFailReason = _("Insufficient funds.");
                    } else if (credit_type == ONLY_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 USDA.");
                    } else if (credit_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 USDA.");
                    } else {
                        strFailReason = _("Unable to locate enough Obfuscation denominated funds for this transaction.");
                        strFailReason += " " + _("Obfuscation uses exact denominated amounts to send funds, you might simply need to anonymize some more credits.");
                    }
                ...
                BOOST_FOREACH (PAIRTYPE(const CWalletTx*, unsigned int) pcredit, setCoins) {
                    CAmount nCredit = pcredit.first->vout[pcredit.second].nValue;
                    //The credit age after the next block (depth+1) is used instead of the current,
                    //reflecting an assumption the user would accept a bit more delay for
                    //a chance at a free transaction.
                    //But mempool inputs might still be in the mempool, so their age stays 0
                    int age = pcredit.first->GetDepthInMainChain();
                    if (age != 0)
                        age += 1;
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-advantage-address
                    CScript scriptChange;

                    // credit control: send change to custom address
                    if (creditControl && !boost::get<CNoDestination>(&creditControl->destChange))
                        scriptChange = GetScriptForDestination(creditControl->destChange);

                    // no credit control: send change to newly generated address
                    else {
                ...
                // Fill vin
                BOOST_FOREACH (const PAIRTYPE(const CWalletTx*, unsigned int) & credit, setCoins)
                    txNew.vin.push_back(CTxIn(credit.first->GetHash(), credit.second));

                // Sign
                int nIn = 0;
                BOOST_FOREACH (const PAIRTYPE(const CWalletTx*, unsigned int) & credit, setCoins)
                    if (!SignSignature(*this, *credit.first, txNew, nIn++)) {
...
bool CWallet::CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* creditControl, AvailableCoinsType credit_type, bool useIX, CAmount nFeePay)
{
    vector<pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, creditControl, credit_type, useIX, nFeePay);
}

// ppcredit: create credit stake transaction
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    BOOST_FOREACH (PAIRTYPE(const CWalletTx*, unsigned int) pcredit, setStakeCoins) {
        //make sure that enough time has elapsed between
        CBlockIndex* pindex = NULL;
        BlockMap::iterator it = mapBlockIndex.find(pcredit.first->hashBlock);
        ...
        COutPoint prevoutStake = COutPoint(pcredit.first->GetHash(), pcredit.second);
        nTxNewTime = GetAdjustedTime();

        //iterates each utxo inside of CheckStakeKernelHash()
        if (CheckStakeKernelHash(nBits, block, *pcredit.first, prevoutStake, nTxNewTime, nHashDrift, false, hashProofOfStake, true)) {
            ...
            // Found a kernel
            if (fDebug && GetBoolArg("-printcreditstake", false))
                LogPrintf("CreateCoinStake : kernel found\n");

            vector<valtype> vSolutions;
            txnouttype whichType;
            CScript scriptPubKeyOut;
            scriptPubKeyKernel = pcredit.first->vout[pcredit.second].scriptPubKey;
            ...
            txNew.vin.push_back(CTxIn(pcredit.first->GetHash(), pcredit.second));
            nCredit += pcredit.first->vout[pcredit.second].nValue;
            vwtxPrev.push_back(pcredit.first);
            txNew.vout.push_back(CTxOut(0, scriptPubKeyOut));

            //presstab HyperStake - calculate the total size of our new output including the stake reward so that we can use it to decide whether to split the stake outputs
            const CBlockIndex* pIndex0 = chainActive.Tip();
            uint64_t nTotalSize = pcredit.first->vout[pcredit.second].nValue + GetBlockValue(pIndex0->nHeight);

            //presstab HyperStake - if MultiSend is set to send in creditstake we will add our outputs here (values asigned further down)
            if (nTotalSize / 2 > nStakeSplitThreshold * CREDIT)
    ...
    while (true) {
        ...
        if (nBytes >= DEFAULT_BLOCK_MAX_SIZE / 5)
            return error("CreateCoinStake : exceeded creditstake size limit");
        ...
        if (nMinFee < nFeeNeeded) {
            ...
            if (fDebug)
                LogPrintf("CreateCoinStake : fee for creditstake %s\n", FormatMoney(nMinFee).c_str());
            break;
        }
    }
    ...
    // Sign
    int nIn = 0;
    BOOST_FOREACH (const CWalletTx* pcredit, vwtxPrev) {
        if (!SignSignature(*this, *pcredit, txNew, nIn++))
            return error("CreateCoinStake : failed to sign creditstake");
    }

    // Successfully generated creditstake
...
{
    {
        ...
        LogPrintf("CommitTransaction:\n%s", wtxNew.ToString());
        {
            ...
            // Notify that old credits are spent
            set<uint256> updated_hahes;
            BOOST_FOREACH (const CTxIn& txin, wtxNew.vin) {
                // notify only once
                if (updated_hahes.find(txin.prevout.hash) != updated_hahes.end()) continue;

                CWalletTx& credit = mapWallet[txin.prevout.hash];
                credit.BindWallet(this);
...
string CWallet::PrepareObfuscationDenominate(int minRounds, int maxRounds)
{
    ...
    /*
        Select the credits we'll use

        if minRounds >= 0 it means only denominated inputs are going in and coming out
    */
    if (minRounds >= 0) {
        if (!SelectCoinsByDenominations(obfuScationPool.sessionDenom, 0.1 * CREDIT, OBFUSCATION_POOL_MAX, vCoins, vCoins2, nValueIn, minRounds, maxRounds))
    ...
    */
    ...
    while (nStep < nStepsMax) {
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            ...
            bool fAccepted = false;
            if ((obfuScationPool.sessionDenom & (1 << 0)) && v == ((10000 * CREDIT) + 10000000)) {
                fAccepted = true;
            } else if ((obfuScationPool.sessionDenom & (1 << 1)) && v == ((1000 * CREDIT) + 1000000)) {
                fAccepted = true;
            } else if ((obfuScationPool.sessionDenom & (1 << 2)) && v == ((100 * CREDIT) + 100000)) {
                fAccepted = true;
            } else if ((obfuScationPool.sessionDenom & (1 << 3)) && v == ((10 * CREDIT) + 10000)) {
                fAccepted = true;
            } else if ((obfuScationPool.sessionDenom & (1 << 4)) && v == ((1 * CREDIT) + 1000)) {
                fAccepted = true;
            } else if ((obfuScationPool.sessionDenom & (1 << 5)) && v == ((.1 * CREDIT) + 100)) {
                fAccepted = true;
            }
            if (!fAccepted) continue;
    ...
    if (obfuScationPool.GetDenominations(vOut) != obfuScationPool.sessionDenom) {
        // unlock used credits on failure
        LOCK(cs_wallet);
...
std::map<CTxDestination, CAmount> CWallet::GetAddressBalances()
{
...

    {
        ...
        BOOST_FOREACH (PAIRTYPE(uint256, CWalletTx) walletEntry, mapWallet) {
            CWalletTx* pcredit = &walletEntry.second;

            if (!IsFinalTx(*pcredit) || !pcredit->IsTrusted())
                continue;

            if (pcredit->IsCoinBase() && pcredit->GetBlocksToMaturity() > 0)
                continue;

            int nDepth = pcredit->GetDepthInMainChain();
            if (nDepth < (pcredit->IsFromMe(ISMINE_ALL) ? 0 : 1))
                continue;

            for (unsigned int i = 0; i < pcredit->vout.size(); i++) {
                CTxDestination addr;
                if (!IsMine(pcredit->vout[i]))
                    continue;
                if (!ExtractDestination(pcredit->vout[i].scriptPubKey, addr))
                    continue;

                CAmount n = IsSpent(walletEntry.first, i) ? 0 : pcredit->vout[i].nValue;
...
set<set<CTxDestination> > CWallet::GetAddressGroupings()
{
    ...
    BOOST_FOREACH (PAIRTYPE(uint256, CWalletTx) walletEntry, mapWallet) {
        ...
        if (pcredit->vin.size() > 0) {
            bool any_mine = false;
            // group all input addresses with each other
            BOOST_FOREACH (CTxIn txin, pcredit->vin) {
                CTxDestination address;
            ...
            if (any_mine) {
                BOOST_FOREACH (CTxOut txout, pcredit->vout)
                    if (IsChange(txout)) {
        ...
        for (unsigned int i = 0; i < pcredit->vout.size(); i++)
            if (IsMine(pcredit->vout[i])) {
                CTxDestination address;
                if (!ExtractDestination(pcredit->vout[i].scriptPubKey, address))
                    continue;
                grouping.insert(address);
                groupings.insert(grouping);
                grouping.clear();
...
void CWallet::AutoCombineDust()
{
    ...
    map<CBitcreditAddress, vector<COutput> > mapCoinsByAddress = AvailableCoinsByAddress(true, 0);

    //credits are sectioned by address. This combination code only wants to combine inputs that belong to the same address
    for (map<CBitcreditAddress, vector<COutput> >::iterator it = mapCoinsByAddress.begin(); it != mapCoinsByAddress.end(); it++) {
        vector<COutput> vCoins, vRewardCoins;
        vCoins = it->second;

        //find masternode rewards that need to be combined
        CCoinControl* creditControl = new CCoinControl();
        CAmount nTotalRewardsValue = 0;
        BOOST_FOREACH (const COutput& out, vCoins) {
            //no credits should get this far if they dont have proper maturity, this is double checking
            if (out.tx->IsCoinStake() && out.tx->GetDepthInMainChain() < CREDITBASE_MATURITY + 1)
                continue;

            if (out.Value() > nAutoCombineThreshold * CREDIT)
                continue;

            COutPoint outpt(out.tx->GetHash(), out.i);
            creditControl->Select(outpt);
            vRewardCoins.push_back(out);
            nTotalRewardsValue += out.Value();
        }

        //if no inputs found then return
        if (!creditControl->HasSelected())
            continue;

        //we cannot combine one credit with itself
        ...
        CWalletTx wtx;
        CReserveKey keyChange(this); // this change address does not end up being used, because change is returned with credit control switch
        string strErr;
        CAmount nFeeRet = 0;

        //get the fee amount
        CWalletTx wtxdummy;
        CreateTransaction(vecSend, wtxdummy, keyChange, nFeeRet, strErr, creditControl, ALL_CREDITS, false, CAmount(0));
        vecSend[0].second = nTotalRewardsValue - nFeeRet - 500;

        if (!CreateTransaction(vecSend, wtx, keyChange, nFeeRet, strErr, creditControl, ALL_CREDITS, false, CAmount(0))) {
...
bool CWallet::MultiSend()
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (out.tx->GetDepthInMainChain() != CREDITBASE_MATURITY + 1)
            continue;
        ...
        if (vDisabledAddresses.size() > 0) {
            for (unsigned int i = 0; i < vDisabledAddresses.size(); i++) {
                if (vDisabledAddresses[i] == CBitcreditAddress(destMyAddress).ToString()) {
        ...
        // create new credit control, populate it with the selected utxo, create sending vector
        ...
        for (unsigned int i = 0; i < vMultiSend.size(); i++) {
            ...
            CBitcreditAddress strAddSend(vMultiSend[i].first);
        ...
        CWalletTx wtxdummy;
        string strErr;
        CreateTransaction(vecSend, wtxdummy, keyChange, nFeeRet, strErr, cControl, ALL_CREDITS, false, CAmount(0));
        ...
        // Create the transaction and commit it to the network
        if (!CreateTransaction(vecSend, wtx, keyChange, nFeeRet, strErr, cControl, ALL_CREDITS, false, CAmount(0))) {
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, (Params().CREDITBASE_MATURITY() + 1) - GetDepthInMainChain());
}
