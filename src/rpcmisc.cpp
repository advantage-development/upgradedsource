...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total advantage balance of the wallet\n"
            "  \"obfuscation_balance\": xxxxxx, (numeric) the anonymized advantage balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in advantage/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in advantage/kb\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"advantageaddress\"\n"
            "\nReturn information about the given advantage address.\n"
            "\nArguments:\n"
            "1. \"advantageaddress\"     (string, required) The advantage address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"advantageaddress\", (string) The advantage address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Advantage address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are advantage addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) advantage address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"advantageaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"advantageaddress\"  (string, required) The advantage address to use for the signature.\n"
...
Value getstakingstatus(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"mintablecredits\": true|false,      (boolean) if the wallet has mintable credits\n"
            "  \"enoughcredits\": true|false,        (boolean) if available credits are greater than reserve balance\n"
    ...
    if (pwalletMain) {
        ...
        obj.push_back(Pair("mintablecredits", pwalletMain->MintableCoins()));
        obj.push_back(Pair("enoughcredits", nReserveBalance <= pwalletMain->GetBalance()));
    }
