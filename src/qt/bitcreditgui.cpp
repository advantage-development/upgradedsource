...
BitcreditGUI::BitcreditGUI(const NetworkStyle* networkStyle, QWidget* parent) : QMainWindow(parent),
                                                                            clientModel(0),
                                                                            walletFrame(0),
                                                                            ...
                                                                            spinnerFrame(0)
{
    ...
    QString windowTitle = tr("Advantage Core") + " - ";
#ifdef ENABLE_WALLET
...
void BitcreditGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send credits to a Advantage address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and advantage: URIs)"));
    ...
    pioAction = new QAction(QIcon(":/icons/bitcredit"), tr("&Advantage Website"), this);
        pioAction->setToolTip(tr("Visit the Official Advantage website"));
    bitcredittalkAction = new QAction(QIcon(":/icons/bitcredit"), tr("&Textile Print"), this);
        bitcredittalkAction->setToolTip(tr("Advantage Own Products To Print Textile"));
    shopAction = new QAction(QIcon(":/icons/bitcredit"), tr("&My Shop"), this);
        shopAction->setToolTip(tr("Advantage Own Shop"));
    chainAction = new QAction(QIcon(":/icons/bitcredit"), tr("&Advantage Discord"), this);
        chainAction->setToolTip(tr("Join Advantage Discord"));
    explorerAction = new QAction(QIcon(":/icons/bitcredit"), tr("&Advantage Explorer"), this);
        explorerAction->setToolTip(tr("Advantage Explorer"));
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About Advantage Core"), this);
    aboutAction->setStatusTip(tr("Show information about Advantage Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Advantage"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Advantage addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Advantage addresses"));
    ...
    openAction->setStatusTip(tr("Open a Advantage: URI or payment request"));
    openBlockExplorerAction = new QAction(QIcon(":/icons/explorer"), tr("&Blockchain explorer"), this);
    openBlockExplorerAction->setStatusTip(tr("Block explorer window"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Advantage Core help message to get a list with possible Advantage command-line options"));

    connect(quitAction, SIGNAL(triggered()), qApp, SLOT(quit()));
    connect(aboutAction, SIGNAL(triggered()), this, SLOT(aboutClicked()));
        connect(pioAction, SIGNAL(triggered()), this, SLOT(pioClicked()));
        connect(bitcredittalkAction, SIGNAL(triggered()), this, SLOT(bitcredittalkClicked()));
        connect(shopAction, SIGNAL(triggered()), this, SLOT(shopClicked()));
...
void BitcreditGUI::createMenuBar()
{
    ...
    if (walletFrame) {
        ...
        tools->addAction(pioAction);
        tools->addAction(bitcredittalkAction);
        tools->addAction(shopAction);
        tools->addAction(chainAction);
...
void BitcreditGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Advantage Core client") + " " + networkStyle->getTitleAddText();
...
void BitcreditGUI::pioClicked()
{
    QDesktopServices::openUrl(QUrl("http://advantage.tech/"));
}

void BitcreditGUI::bitcredittalkClicked()
{
    QDesktopServices::openUrl(QUrl("https://textile.advantage.tech/"));
}

void BitcreditGUI::shopClicked()
{
    QDesktopServices::openUrl(QUrl("https://goo.gl/TLng3u"));
}

void BitcreditGUI::chainClicked()
{
    QDesktopServices::openUrl(QUrl("https://discord.gg/qaneFPm"));
}
void BitcreditGUI::explorerClicked()
{
    QDesktopServices::openUrl(QUrl("http://explorer.advantage.tech/"));
}
...
void BitcreditGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void BitcreditGUI::gotoBip38Tool()
{
    if (walletFrame) walletFrame->gotoBip38Tool();
}

void BitcreditGUI::gotoMultiSendDialog()
{
...
void BitcreditGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Advantage network", "", count));
}
...
void BitcreditGUI::setNumBlocks(int count)
{
    ...
    //    if(secs < 25*60) // 90*60 for bitcredit but we are 4x times faster
...
{
    QString strTitle = tr("Advantage Core"); // default title
...
void BitcreditGUI::setStakingStatus()
{
    if (pwalletMain)
...
void BitcreditGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
...
void UnitDisplayStatusBarControl::updateDisplayUnit(int newUnits)
{
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        setPixmap(QIcon(":/icons/unit_" + BitcreditUnits::id(newUnits)).pixmap(39, STATUSBAR_ICONSIZE));
    } else {
        setPixmap(QIcon(":/icons/unit_t" + BitcreditUnits::id(newUnits)).pixmap(39, STATUSBAR_ICONSIZE));
    }
}
