...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Advantage address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no Advantage: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcreditUnits::parse(BitcreditUnits::USDA, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert advantage:// to advantage:
    //
    //    Cannot handle this later, because advantage:// will cause Qt to see the part after // as host,
...
QString formatBitcreditURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcreditUnits::format(BitcreditUnits::USDA, info.amount, false, BitcreditUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open advantage.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Advantage.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Advantage.lnk
...
{
    return GetAutostartDir() / "advantage.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a advantage.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Advantage\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the advantage app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add advantage app to startup item list
