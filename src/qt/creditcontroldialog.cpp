...
#include "ui_creditcontroldialog.h"
...
CCoinControl* CoinControlDialog::creditControl = new CCoinControl();
...
// click on header
    ...
    // change credit control first column label due Qt4 bug.
    // see https://github.com/bitcredit/bitcredit/issues/5716
...
void CoinControlDialog::buttonSelectAllClicked()
{
    ...
    if (state == Qt::Unchecked)
        creditControl->UnSelectAll(); // just to be sure
    CoinControlDialog::updateLabels(model, this);
}
...
{
    GUIUtil::setClipboard(BitcreditUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
// context menu action: lock credit
...
// context menu action: unlock credit
...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem* item, int column)
{
    if (column == COLUMN_CHECKBOX && item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64 characters (this means its a child node, so its not a parent node in tree mode)
    {
        ...
        if (item->checkState(COLUMN_CHECKBOX) == Qt::Unchecked)
            ...
            if (creditControl->useObfuScation && rounds < nObfuscationRounds) {
                ...
                creditControl->useObfuScation = false;
            }
        }
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    creditControl->ListSelected(vCoinControl);
    ...
    if (nQuantity > 0) {
        ...
        // IX Fee
        if (creditControl->useSwiftTX) nPayFee = max(nPayFee, CENT);
        // Allow free?
        ...
        if (nPayAmount > 0) {
            ...
            // DS Fee = overpay
            if (creditControl->useObfuScation && nChange > 0) {
                nPayFee += nChange;
                nChange = 0;
            }
    ...
    int nDisplayUnit = BitcreditUnits::USDA;
    ...
    l3->setText(BitcreditUnits::formatWithUnit(nDisplayUnit, nPayFee));   // Fee
    l4->setText(BitcreditUnits::formatWithUnit(nDisplayUnit, nAfterFee)); // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));   // Bytes
    l6->setText(sPriorityLabel);                                        // Priority
    l7->setText(fDust ? tr("yes") : tr("no"));                          // Dust
    l8->setText(BitcreditUnits::formatWithUnit(nDisplayUnit, nChange));   // Change
    ...
    toolTip1 += tr("This means a fee of at least %1 per kB is required.").arg(BitcreditUnits::formatWithUnit(nDisplayUnit, CWallet::minTxFee.GetFeePerK())) + "<br /><br />";
    toolTip1 += tr("Can vary +/- 1 byte per input.");

    QString toolTip2 = tr("Transactions with higher priority are more likely to get included into a block.") + "<br /><br />";
    toolTip2 += tr("This label turns red, if the priority is smaller than \"medium\".") + "<br /><br />";
    toolTip2 += tr("This means a fee of at least %1 per kB is required.").arg(BitcreditUnits::formatWithUnit(nDisplayUnit, CWallet::minTxFee.GetFeePerK()));

    QString toolTip3 = tr("This label turns red, if any recipient receives an amount smaller than %1.").arg(BitcreditUnits::formatWithUnit(nDisplayUnit, ::minRelayTxFee.GetFee(546)));
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) credits, mapCoins) {
        QTreeWidgetItem* itemWalletAddress = new QTreeWidgetItem();
        itemWalletAddress->setCheckState(COLUMN_CHECKBOX, Qt::Unchecked);
        QString sWalletAddress = credits.first;
        ...
        int nChildren = 0;
        int nInputSum = 0;
        BOOST_FOREACH (const COutput& out, credits.second) {
            int nInputSize = 0;
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                sAddress = QString::fromStdString(CBitcreditAddress(outputAddress).ToString());

                // if listMode or change => show Advantage address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            itemOutput->setToolTip(COLUMN_AMOUNT, BitcreditUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
            ...
            // disable locked credits
            if (model->isLockedCoin(txhash, out.i)) {
                COutPoint outpt(txhash, out.i);
                creditControl->UnSelect(outpt); // just to be sure
                itemOutput->setDisabled(true);
                itemOutput->setIcon(COLUMN_CHECKBOX, QIcon(":/icons/lock_closed"));
            }

            // set checkbox
            if (creditControl->IsSelected(txhash, out.i))
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setToolTip(COLUMN_AMOUNT, BitcreditUnits::format(nDisplayUnit, nSum));
