...
{
    return QCoreApplication::translate("advantage-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator)
{
    ...
    // Load e.g. bitcredit_de.qm (shortcut "de" needs to be defined in advantage.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcredit_de_DE.qm (shortcut "de_DE" needs to be defined in advantage.qrc)
...
/** Main Advantage application object */
...
#include "advantage.moc"

BitcreditCore::BitcreditCore() : QObject()
{
}
...
void BitcreditCore::restart(QStringList args)
{
...
BitcreditApplication::BitcreditApplication(int& argc, char** argv) : QApplication(argc, argv),
                                                                 coreThread(0),
                                                                 optionsModel(0),
                                                                 clientModel(0),
                                                                 window(0),
...
void BitcreditApplication::createWindow(const NetworkStyle* networkStyle)
{
    window = new BitcreditGUI(networkStyle, 0);
...
void BitcreditApplication::createSplashScreen(const NetworkStyle* networkStyle)
{
...
void BitcreditApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            connect(walletModel, SIGNAL(creditsSent(CWallet*, SendCoinsRecipient, QByteArray)),
...
{
    QMessageBox::critical(0, "Runaway exception", BitcreditGUI::tr("A fatal error occurred. Advantage can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    Q_INIT_RESOURCE(advantage_locale);
    Q_INIT_RESOURCE(advantage);
    ...
    if (!boost::filesystem::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Advantage Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Advantage Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    try {
        ...
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Advantage Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
