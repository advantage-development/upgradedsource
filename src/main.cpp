...
/** Constant stuff for creditbase transactions we create: */
CScript CREDITBASE_FLAGS;
...
// XX42    boost::signals2::signal<void(const uint256&)> EraseTransaction;
    /** Notifies listeners of an updated transaction without new data (for now: a creditbase potentially becoming visible). */
...
CCoinsViewCache* pcreditsTip = NULL;
...
int GetInputAge(CTxIn& vin)
{
    ...
    {
        LOCK(mempool.cs);
        CCoinsViewMemPool viewMempool(pcreditsTip, mempool);
        view.SetBackend(viewMempool); // temporarily switch cache backend to db+mempool view

        const CCoins* credits = view.AccessCoins(vin.prevout.hash);

        if (credits) {
            if (credits->nHeight < 0) return 0;
            return (chainActive.Tip()->nHeight + 1) - credits->nHeight;
        } else
            return -1;
    }
}
...
// ppcredit: total credit age spent in transaction, in the unit of credit-days.
// Only those credits meeting minimum age requirement counts. As those
// transactions not in main chain are not currently indexed so we
// might not find out about their credit age. Older transactions are
// guaranteed to be in main chain by sync-checkpoint. This rule is
// introduced to help nodes establish a consistent view of the credit
// age (trust score) of competing branches.
bool GetCoinAge(const CTransaction& tx, const unsigned int nTxTime, uint64_t& nCoinAge)
{
    uint256 bnCentSecond = 0; // credit age in the unit of cent-seconds
    nCoinAge = 0;
    ...
    BOOST_FOREACH (const CTxIn& txin, tx.vin) {
        ...
        if (prevblock.nTime + nStakeMinAge > nTxTime)
            continue; // only count credits meeting min age requirement
    ...
    uint256 bnCoinDay = bnCentSecond / CREDIT / (24 * 60 * 60);
    LogPrintf("credit age bnCoinDay=%s\n", bnCoinDay.ToString().c_str());
...
bool CheckTransaction(const CTransaction& tx, CValidationState& state)
{
    ...
    if (tx.IsCoinBase()) {
        if (tx.vin[0].scriptSig.size() < 2 || tx.vin[0].scriptSig.size() > 150)
            return state.DoS(100, error("CheckTransaction() : creditbase script size=%d", tx.vin[0].scriptSig.size()),
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (tx.IsCoinBase())
        return state.DoS(100, error("AcceptToMemoryPool: : creditbase as individual tx"),
            REJECT_INVALID, "creditbase");

    //Coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
        return state.DoS(100, error("AcceptToMemoryPool: creditstake as individual tx"),
            REJECT_INVALID, "creditstake");
...

    {
        ...
        {
            LOCK(pool.cs);
            CCoinsViewMemPool viewMemPool(pcreditsTip, pool);
        ...
        // sigops, making it impossible to mine. Since the creditbase transaction
        ...
        if (mapObfuscationBroadcastTxes.count(hash)) {
            mempool.PrioritiseTransaction(hash, hash.ToString(), 1000, 0.1 * CREDIT);
        } else if (!ignoreFees) {
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool isDSTX)
{
    ...
    // Coinbase is only valid in a block, not as a loose transaction
    if (tx.IsCoinBase())
        return state.DoS(100, error("AcceptableInputs: : creditbase as individual tx"),
            REJECT_INVALID, "creditbase");
...
bool GetTransaction(const uint256& hash, CTransaction& txOut, uint256& hashBlock, bool fAllowSlow)
{
    CBlockIndex* pindexSlow = NULL;
    {
        ...
        if (fAllowSlow) { // use credit database to locate block that contains transaction, and scan it
            int nHeight = -1;
            {
                CCoinsViewCache& view = *pcreditsTip;
                const CCoins* credits = view.AccessCoins(hash);
                if (credits)
                    nHeight = credits->nHeight;
            }
            if (nHeight > 0)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 0 * CREDIT;
    } else if (nHeight < 140001) {
        nSubsidy = 0 * CREDIT;
    } else if (nHeight < 160001) {
        nSubsidy = 0 * CREDIT;
    } else if (nHeight < 180001) {
        nSubsidy = 0 * CREDIT;
    } else if (nHeight < 210001) {
        nSubsidy = 0 * CREDIT;
    } else if (nHeight < 240001) {
        nSubsidy = 0 * CREDIT;
    } else if (nHeight < 270001) {
        nSubsidy = 0 * CREDIT;
    } else if (nHeight < 300001) {
        nSubsidy = 0 * CREDIT;
    } else if (nHeight < 340001) {
        nSubsidy = 0 * CREDIT;
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
    if (!tx.IsCoinBase()) {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            const CCoins* credits = inputs.AccessCoins(prevout.hash);
            assert(credits);

            // If prev is creditbase, check that it's matured
            if (credits->IsCoinBase() || credits->IsCoinStake()) {
                if (nSpendHeight - credits->nHeight < Params().CREDITBASE_MATURITY())
                    return state.Invalid(
                        error("CheckInputs() : tried to spend creditbase at depth %d, creditstake=%d", nSpendHeight - credits->nHeight, credits->IsCoinStake()),
                        REJECT_INVALID, "bad-txns-premature-spend-of-creditbase");
            }

            // Check for negative or overflow input values
            nValueIn += credits->vout[prevout.n].nValue;
            if (!MoneyRange(credits->vout[prevout.n].nValue) || !MoneyRange(nValueIn))
        ...
        if (fScriptChecks) {
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                // Verify signature
                CScriptCheck check(*credits, tx, i, flags, cacheStore);
                if (pvChecks) {
                    ...
                    if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        // non-upgraded nodes.
                        CScriptCheck check(*credits, tx, i,
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        if (i > 0) { // not creditbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                CCoinsModifier credits = view.ModifyCoins(out.hash);
                if (undo.nHeight != 0) {
                    // undo data contains height: this is the last output of the prevout tx being spent
                    if (!credits->IsPruned())
                        fClean = fClean && error("DisconnectBlock() : undo data overwriting existing transaction");
                    credits->Clear();
                    credits->fCoinBase = undo.fCoinBase;
                    credits->nHeight = undo.nHeight;
                    credits->nVersion = undo.nVersion;
                } else {
                    if (credits->IsPruned())
                        fClean = fClean && error("DisconnectBlock() : undo data adding output to missing transaction");
                }
                if (credits->IsAvailable(out.n))
                    fClean = fClean && error("DisconnectBlock() : undo data overwriting existing output");
                if (credits->vout.size() < out.n + 1)
                    credits->vout.resize(out.n + 1);
                credits->vout[out.n] = undo.txout;
            }
        }
    }
...
{
    RenameThread("advantage-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // If such overwrites are allowed, creditbases and transactions depending upon those
    ...
    if (fEnforceBIP30) {
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
            const CCoins* credits = view.AccessCoins(tx.GetHash());
            if (credits && !credits->IsPruned())
    ...
    // ppcredit: track money supply and mint amount info
    ...
    // Watch for changes to the previous creditbase transaction.
...
bool static FlushStateToDisk(CValidationState& state, FlushStateMode mode)
{
    ...
    try {
        if ((mode == FLUSH_STATE_ALWAYS) ||
            ((mode == FLUSH_STATE_PERIODIC || mode == FLUSH_STATE_IF_NEEDED) && pcreditsTip->GetCacheSize() > nCoinCacheSize) ||
            ...
            if (!CheckDiskSpace(100 * 2 * 2 * pcreditsTip->GetCacheSize()))
            ...
            if (!pcreditsTip->Flush())
                return state.Abort("Failed to write to credit database");
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    LogPrintf("UpdateTip: new best=%s  height=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f  cache=%u\n",
        ...
        DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()),
        Checkpoints::GuessVerificationProgress(chainActive.Tip()), (unsigned int)pcreditsTip->GetCacheSize());
...
bool static DisconnectTip(CValidationState& state)
{
    ...
    mempool.removeCoinbaseSpends(pcreditsTip, pindexDelete->nHeight);
    mempool.check(pcreditsTip);
...
{
    // All modifications to the credit state will be done in this cache.
    // Only when all have succeeded, we push it to pcreditsTip.
    //    CCoinsViewCache view(*pcreditsTip, true);
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    if (miPrev != mapBlockIndex.end()) {
        ...
        // ppcredit: compute chain trust score
        pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

        // ppcredit: compute stake entropy bit for stake modifier
        if (!pindexNew->SetStakeEntropyBit(pindexNew->GetStakeEntropyBit()))
            LogPrintf("AddToBlockIndex() : SetStakeEntropyBit() failed \n");

        // ppcredit: record proof-of-stake hash value
        if (pindexNew->IsProofOfStake()) {
        ...
        // ppcredit: compute stake modifier
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // First transaction must be creditbase, the rest must not be
    if (block.vtx.empty() || !block.vtx[0].IsCoinBase())
        return state.DoS(100, error("CheckBlock() : first tx is not creditbase"),
            REJECT_INVALID, "bad-cb-missing");
    for (unsigned int i = 1; i < block.vtx.size(); i++)
        if (block.vtx[i].IsCoinBase())
            return state.DoS(100, error("CheckBlock() : more than one creditbase"),
                REJECT_INVALID, "bad-cb-multiple");

    if (block.IsProofOfStake()) {
        // Coinbase output should be empty if proof-of-stake block
        if (block.vtx[0].vout.size() != 1 || !block.vtx[0].vout[0].IsEmpty())
            return state.DoS(100, error("CheckBlock() : creditbase output not empty for proof-of-stake block"));

        // Second transaction must be creditstake, the rest must not be
        if (block.vtx.empty() || !block.vtx[1].IsCoinStake())
            return state.DoS(100, error("CheckBlock() : second tx is not creditstake"));
        for (unsigned int i = 2; i < block.vtx.size(); i++)
            if (block.vtx[i].IsCoinStake())
                return state.DoS(100, error("CheckBlock() : more than one creditstake"));
    }
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    // Enforce block.nVersion=2 rule that the creditbase starts with serialized block height
    ...
    if (block.nVersion >= 2 &&
        ...
        if (block.vtx[0].vin[0].scriptSig.size() < expect.size() ||
            ...
            return state.DoS(100, error("%s : block height mismatch in creditbase", __func__), REJECT_INVALID, "bad-cb-height");
        }
    }
...
bool TestBlockValidity(CValidationState& state, const CBlock& block, CBlockIndex* const pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    CCoinsViewCache viewNew(pcreditsTip);
...
bool static LoadBlockIndexDB()
{
    ...
    if (!fLastShutdownWasPrepared && !GetBoolArg("-forcestart", false) && !GetBoolArg("-reindex", false) && (vSortedByHeight.size() != vinfoBlockFile[nLastBlockFile].nHeightLast + 1) && (vinfoBlockFile[nLastBlockFile].nHeightLast != 0)) {
        ...
        //fix Assertion `hashPrevBlock == view.GetBestBlock()' failed. By adjusting height to the last recorded by creditsview
        CBlockIndex* pindexCoinsView = mapBlockIndex[pcreditsTip->GetBestBlock()];
        ...
        if (pcreditsTip->GetBestBlock() != vSortedByHeight[vSortedByHeight.size() - 1].second->GetBlockHash()) {
            isFixed = false;
            strError = "pcreditsTip best block is not correct";
        }
    ...
    BlockMap::iterator it = mapBlockIndex.find(pcreditsTip->GetBestBlock());
    if (it == mapBlockIndex.end())
        return true;
...
bool CVerifyDB::VerifyDB(CCoinsView* creditsview, int nCheckLevel, int nCheckDepth)
{
    LOCK(cs_main);
    ...
    CCoinsViewCache credits(creditsview);
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (nCheckLevel >= 3 && pindex == pindexState && (credits.GetCacheSize() + pcreditsTip->GetCacheSize()) <= nCoinCacheSize) {
            bool fClean = true;
            if (!DisconnectBlock(block, state, pindex, credits, &fClean))
    ...
    if (pindexFailure)
        return error("VerifyDB() : *** credit database inconsistencies found (last %i blocks, %i good transactions before that)\n", chainActive.Height() - pindexFailure->nHeight + 1, nGoodTransactions);
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            if (!ReadBlockFromDisk(block, pindex))
                return error("VerifyDB() : *** ReadBlockFromDisk failed at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
            if (!ConnectBlock(block, state, pindex, credits))
                return error("VerifyDB() : *** found unconnectable block at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
        }
    }

    LogPrintf("No credit database inconsistencies in last %i blocks (%i transactions)\n", chainActive.Height() - pindexState->nHeight, nGoodTransactions);
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_TX: {
        ...
        return txInMap || mapOrphanTransactions.count(inv.hash) ||
               pcreditsTip->HaveCoins(inv.hash);
    }
    case MSG_DSTX:
...
{
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && fFetch /*&& !fImporting*/ && !fReindex) {
            ...
            if (nSyncStarted == 0 || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 6 * 60 * 60) { // NOTE: was "close to today" and 24h in Bitcredit
