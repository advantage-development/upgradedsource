...
#ifndef BITCREDIT_WALLET_H
#define BITCREDIT_WALLET_H
...
static const CAmount nHighTransactionFeeWarning = 1000000 * CREDIT;
//! -maxtxfee default
static const CAmount DEFAULT_TRANSACTION_MAXFEE = 100000000 * CREDIT;
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 USDA at the same time
...
{
private:
    bool SelectCoins(const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* creditControl = NULL, AvailableCoinsType credit_type = ALL_CREDITS, bool useIX = true) const;
    //it was public bool SelectCoins(int64_t nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl *creditControl = NULL, AvailableCoinsType credit_type=ALL_CREDITS, bool useIX = true) const;
...
public:
    ...
    void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed = true, const CCoinControl* creditControl = NULL, bool fIncludeZeroValue = false, AvailableCoinsType nCoinType = ALL_CREDITS, bool fUseIX = false) const;
    std::map<CBitcreditAddress, std::vector<COutput> > AvailableCoinsByAddress(bool fConfirmed = true, CAmount maxCoinValue = 0);
    ...
    bool CreateTransaction(const std::vector<std::pair<CScript, CAmount> >& vecSend,
        ...
        const CCoinControl* creditControl = NULL,
        AvailableCoinsType credit_type = ALL_CREDITS,
        bool useIX = false,
        CAmount nFeePay = 0);
    bool CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* creditControl = NULL, AvailableCoinsType credit_type = ALL_CREDITS, bool useIX = false, CAmount nFeePay = 0);
...
public:
    ...
    {
        // Must wait until creditbase is safely deep enough in the chain before valuing it
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == Params().MasternodeCollateralLimit() * CREDIT) continue; // do not count MN-like outputs
...
public:
    ...
    int Priority() const
    {
        ...
        if (tx->vout[i].nValue < 1 * CREDIT) return 20000;

        //nondenom return largest first
        return -(tx->vout[i].nValue / CREDIT);
    }
