...
void CTxMemPool::pruneSpent(const uint256& hashTx, CCoins& credits)
{
    LOCK(cs);
    ...
    while (it != mapNextTx.end() && it->first.hash == hashTx) {
        credits.Spend(it->first.n); // and remove those outputs from credits
        it++;
    }
}
...
void CTxMemPool::removeCoinbaseSpends(const CCoinsViewCache* pcredits, unsigned int nMemPoolHeight)
{
    // Remove transactions spending a creditbase which are now immature
    LOCK(cs);
    ...
    for (std::map<uint256, CTxMemPoolEntry>::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        BOOST_FOREACH (const CTxIn& txin, tx.vin) {
            ...
            if (it2 != mapTx.end())
                continue;
            const CCoins* credits = pcredits->AccessCoins(txin.prevout.hash);
            if (fSanityCheck) assert(credits);
            if (!credits || ((credits->IsCoinBase() || credits->IsCoinStake()) && nMemPoolHeight - credits->nHeight < Params().CREDITBASE_MATURITY())) {
...
void CTxMemPool::check(const CCoinsViewCache* pcredits) const
{
    if (!fSanityCheck)
        return;
    ...
    CCoinsViewCache mempoolDuplicate(const_cast<CCoinsViewCache*>(pcredits));
    ...
    for (std::map<uint256, CTxMemPoolEntry>::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        BOOST_FOREACH (const CTxIn& txin, tx.vin) {
            // Check that every mempool transaction's inputs refer to available credits, or other mempool tx's.
            std::map<uint256, CTxMemPoolEntry>::const_iterator it2 = mapTx.find(txin.prevout.hash);
            if (it2 != mapTx.end()) {
                ...
                const CCoins* credits = pcredits->AccessCoins(txin.prevout.hash);
                assert(credits && credits->IsAvailable(txin.prevout.n));
            }
...
bool CCoinsViewMemPool::GetCoins(const uint256& txid, CCoins& credits) const
{
    ...
    if (mempool.lookup(txid, tx)) {
        credits = CCoins(tx, MEMPOOL_HEIGHT);
        return true;
    }
    return (base->GetCoins(txid, credits) && !credits.IsPruned());
}
