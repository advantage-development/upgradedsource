...
Value getrawmempool(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in advantage\n"
...
Value gettxout(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of advantage addresses\n"
            "     \"advantageaddress\"   	 	(string) advantage address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
            "  \"version\" : n,            (numeric) The version\n"
            "  \"creditbase\" : true|false   (boolean) Coinbase or not\n"
            "}\n"
    ...
    if (fMempool) {
        LOCK(mempool.cs);
        CCoinsViewMemPool view(pcreditsTip, mempool);
        if (!view.GetCoins(hash, credits))
            return Value::null;
        mempool.pruneSpent(hash, credits); // TODO: this should be done by the CCoinsViewMemPool
    } else {
        if (!pcreditsTip->GetCoins(hash, credits))
            return Value::null;
    }
    if (n < 0 || (unsigned int)n >= credits.vout.size() || credits.vout[n].IsNull())
        return Value::null;

    BlockMap::iterator it = mapBlockIndex.find(pcreditsTip->GetBestBlock());
    CBlockIndex* pindex = it->second;
    ret.push_back(Pair("bestblock", pindex->GetBlockHash().GetHex()));
    if ((unsigned int)credits.nHeight == MEMPOOL_HEIGHT)
        ret.push_back(Pair("confirmations", 0));
    else
        ret.push_back(Pair("confirmations", pindex->nHeight - credits.nHeight + 1));
    ret.push_back(Pair("value", ValueFromAmount(credits.vout[n].nValue)));
    Object o;
    ScriptPubKeyToJSON(credits.vout[n].scriptPubKey, o, true);
    ret.push_back(Pair("scriptPubKey", o));
    ret.push_back(Pair("version", credits.nVersion));
    ret.push_back(Pair("creditbase", credits.fCoinBase));
...
Value verifychain(const Array& params, bool fHelp)
{
    ...
    return CVerifyDB().VerifyDB(pcreditsTip, nCheckLevel, nCheckDepth);
}
