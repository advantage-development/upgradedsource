...
void SendMoney(const CTxDestination& address, CAmount nValue, CWalletTx& wtxNew, AvailableCoinsType credit_type = ALL_CREDITS)
{
    // Check amount
    if (nValue <= 0)
    ...
    if (!pwalletMain->CreateTransaction(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired, strError, NULL, credit_type)) {
...
Value obfuscation(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "obfuscation <advantageaddress> <amount>\n"
            "advantageaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "obfuscation <advantageaddress> <amount>\n"
            "advantageaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value getpoolinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) Advantage address of current masternode\n"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in advantage.conf\n"
...
Value listmasternodes(const Array& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode Advantage address\n"
    ...
    BOOST_FOREACH (PAIRTYPE(int, CMasternode) & s, vMasternodeRanks) {
        ...
        if (mn != NULL) {
            if (strFilter != "" && strTxHash.find(strFilter) == string::npos &&
                ...
                CBitcreditAddress(mn->pubKeyCollateralAddress.GetID()).ToString().find(strFilter) == string::npos) continue;
            ...
            obj.push_back(Pair("addr", CBitcreditAddress(mn->pubKeyCollateralAddress.GetID()).ToString()));
...
Value masternodeconnect(const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6: 53211\"") + HelpExampleRpc("masternodeconnect", "\"192.168.0.6: 53211\""));
...
Value masternodecurrent (const Array& params, bool fHelp)
{
    ...
    if (winner) {
        ...
        obj.push_back(Pair("pubkey", CBitcreditAddress(winner->pubKeyCollateralAddress.GetID()).ToString()));
...
Value getmasternodestatus (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) Advantage address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
    ...
    if (pmn) {
        ...
        mnObj.push_back(Pair("addr", CBitcreditAddress(pmn->pubKeyCollateralAddress.GetID()).ToString()));
...
Value getmasternodewinners (const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) Advantage MN Address\n"
