...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, Object& entry)
{
    ...
    BOOST_FOREACH (const CTxIn& txin, tx.vin) {
        Object in;
        if (tx.IsCoinBase())
            in.push_back(Pair("creditbase", HexStr(txin.scriptSig.begin(), txin.scriptSig.end())));
        else {
...
Value getrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "           \"advantageaddress\"        (string) advantage address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
Value listunspent(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of advantage addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) advantage address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"address\" : \"address\",  (string) the advantage address\n"
    ...
    if (params.size() > 2) {
        ...
        BOOST_FOREACH (Value& input, inputs) {
            CBitcreditAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Advantage address: ") + input.get_str());
    ...
    BOOST_FOREACH (const COutput& out, vecOutputs) {
        ...
        CTxDestination address;
        if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, address)) {
            entry.push_back(Pair("address", CBitcreditAddress(address).ToString()));
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the advantage address, the value is the btc amount\n"
            "      ,...\n"
            "    }\n"
...
Value decoderawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) advantage address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
Value decodescript(const Array& params, bool fHelp)
{
    ...
    r.push_back(Pair("p2sh", CBitcreditAddress(CScriptID(script)).ToString()));
    return r;
}
...
Value signrawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        LOCK(mempool.cs);
        CCoinsViewCache& viewChain = *pcreditsTip;
    ...
    if (params.size() > 1 && params[1].type() != null_type) {
        ...
        BOOST_FOREACH (Value& p, prevTxs) {
            ...
            {
                CCoinsModifier credits = view.ModifyCoins(txid);
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        if (credits == NULL || !credits->IsAvailable(txin.prevout.n)) {
            fComplete = false;
            continue;
        }
