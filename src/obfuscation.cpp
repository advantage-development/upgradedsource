...
//
// Unlock credits after Obfuscation fails or succeeds
//
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Advantage and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 USDA per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeAdvantageAmount * CREDIT - pwalletMain->GetAnonymizedBalance();
    ...
    // select credits that should be given to the pool
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nObfuscationRounds)) {
        nValueIn = 0;
        vCoins.clear();

        if (pwalletMain->SelectCoinsDark(nValueMin, 9999999 * CREDIT, vCoins, nValueIn, -2, 0)) {
...
bool CObfuscationPool::SendRandomPaymentToSelf()
{
    ...
    if (nPayment > nBalance) nPayment = nBalance - (0.1 * CREDIT);
    ...
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekey, nFeeRet, strFail, creditControl, ONLY_DENOMINATED);
    if (!success) {
...
bool CObfuscationPool::MakeCollateralAmounts()
{
    ...
    std::string strFail = "";
    vector<pair<CScript, CAmount> > vecSend;
    CCoinControl creditControl;
    creditControl.fAllowOtherInputs = false;
    creditControl.fAllowWatchOnly = false;
    ...
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
        nFeeRet, strFail, &creditControl, ONLY_NONDENOMINATED_NOT10000IFMN);
    if (!success) {
        // if we failed (most likeky not enough funds), try to use all credits instead -
        // MN-like funds should not be touched in any case and we can't mix denominated without collaterals anyway
        CCoinControl* creditControlNull = NULL;
        ...
        success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
            nFeeRet, strFail, creditControlNull, ONLY_NOT10000IFMN);
        if (!success) {
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100USDA+1 ( bit on if present )
    // bit 1 - 10USDA+1
    // bit 2 - 1USDA+1
    // bit 3 - .1USDA+1
    // bit 3 - non-denom
...
int CObfuscationPool::GetDenominationsByAmount(CAmount nAmount, int nDenomTarget)
{
    ...
    BOOST_REVERSE_FOREACH (CAmount v, obfuScationDenominations) {
        if (nDenomTarget != 0) {
            ...
            if ((nDenomTarget & (1 << 0)) && v == ((100 * CREDIT) + 100000)) {
                fAccepted = true;
            } else if ((nDenomTarget & (1 << 1)) && v == ((10 * CREDIT) + 10000)) {
                fAccepted = true;
            } else if ((nDenomTarget & (1 << 2)) && v == ((1 * CREDIT) + 1000)) {
                fAccepted = true;
            } else if ((nDenomTarget & (1 << 3)) && v == ((.1 * CREDIT) + 100)) {
                fAccepted = true;
            }
...
bool CObfuScationSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey)
{
    ...
    if (GetTransaction(vin.prevout.hash, txVin, hash, true)) {
        BOOST_FOREACH (CTxOut out, txVin.vout) {
            if (out.nValue == Params().MasternodeCollateralLimit() * CREDIT) {
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("advantage-obfuscation");
