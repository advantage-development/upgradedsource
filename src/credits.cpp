// Copyright (c) 2012-2014 The Bitcredit developers
...
bool CCoinsView::GetCoins(const uint256& txid, CCoins& credits) const { return false; }
...
bool CCoinsViewBacked::GetCoins(const uint256& txid, CCoins& credits) const { return base->GetCoins(txid, credits); }
...
bool CCoinsViewCache::GetCoins(const uint256& txid, CCoins& credits) const
{
...
CCoinsModifier CCoinsViewCache::ModifyCoins(const uint256& txid)
{
    ...
    if (ret.second) {
        if (!base->GetCoins(txid, ret.first->second.credits)) {
...
bool CCoinsViewCache::HaveCoins(const uint256& txid) const
{
    ...
    return (it != cacheCoins.end() && !it->second.credits.vout.empty());
}
    ...
    for (CCoinsMap::iterator it = mapCoins.begin(); it != mapCoins.end();) {
        if (it->second.flags & CCoinsCacheEntry::DIRTY) { // Ignore non-dirty entries (optimization).
            ...
            if (itUs == cacheCoins.end()) {
                ...
                if ((itUs->second.flags & CCoinsCacheEntry::FRESH) && it->second.credits.IsPruned()) {
...
{
    const CCoins* credits = AccessCoins(input.prevout.hash);
...
bool CCoinsViewCache::HaveInputs(const CTransaction& tx) const
{
    if (!tx.IsCoinBase()) {
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            const CCoins* credits = AccessCoins(prevout.hash);
...
double CCoinsViewCache::GetPriority(const CTransaction& tx, int nHeight) const
{
    ...
    for (const CTxIn& txin:  tx.vin) {
        const CCoins* credits = AccessCoins(txin.prevout.hash);
        assert(credits);
...
CCoinsModifier::~CCoinsModifier()
{
    ...
    if ((it->second.flags & CCoinsCacheEntry::FRESH) && it->second.credits.IsPruned()) {
