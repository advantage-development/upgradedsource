...
#include "config/advantage-config.h"
#endif
...
public:
    ...
    bool GetCoins(const uint256& txid, CCoins& credits) const
    {
        try {
            return CCoinsViewBacked::GetCoins(txid, credits);
...
static CCoinsViewDB* pcreditsdbview = NULL;
static CCoinsViewErrorCatcher* pcreditscatcher = NULL;
...
void PrepareShutdown()
{
    ...
    RenameThread("advantage-shutoff");
...

    {
        ...
        delete pcreditsTip;
        pcreditsTip = NULL;
        delete pcreditscatcher;
        pcreditscatcher = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "advantage.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "advantaged.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"),  53211, 39795));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in USDA/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in USDA/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, creditdb, db, lock, rand, rpc, selectcredits, mempool, net, advantage, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCREDIT_QT)
        debugCategories += ", qt";
...
#ifdef ENABLE_WALLET
    strUsage += HelpMessageOpt("-gen", strprintf(_("Generate credits (default: %u)"), 0));
    strUsage += HelpMessageOpt("-genproclimit=<n>", strprintf(_("Set the number of threads for credit generation if enabled (-1 = all cores, default: %d)"), 1));
#endif
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in USDA/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Advantage specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    if (GetBoolArg("-help-debug", false)) {
        ...
        strUsage += HelpMessageOpt("-printcreditstake", _("Display verbose credit stake messages in the debug.log file."));
    }
#endif
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235: 53211"));
    ...
    strUsage += HelpMessageOpt("-anonymizeadvantageamount=<n>", strprintf(_("Keep N USDA anonymized (default: %u)"), 0));
    strUsage += HelpMessageOpt("-liquidityprovider=<n>", strprintf(_("Provide liquidity to Obfuscation by infrequently mixing credits on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"),  11235, 41211));
    ...
    strUsage += HelpMessageGroup(_("RPC SSL options: (see the Bitcredit Wiki for SSL setup instructions)"));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcredit Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(_("Copyright (C) 2018 The Advantage Core Developers")) + "\n" +
           "\n" +
...
{
    RenameThread("advantage-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Advantage Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Advantage Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    size_t nCoinDBCache = nTotalCache / 2; // use half of the remaining cache for creditdb cache
    nTotalCache -= nCoinDBCache;
    nCoinCacheSize = nTotalCache / 300; // credits in memory require around 300 bytes
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                pcreditsdbview = new CCoinsViewDB(nCoinDBCache, false, fReindex);
                pcreditscatcher = new CCoinsViewErrorCatcher(pcreditsdbview);
                pcreditsTip = new CCoinsViewCache(pcreditscatcher);
                ...
                if (!CVerifyDB().VerifyDB(pcreditsdbview, GetArg("-checklevel", 4),
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Advantage Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Advantage Core to complete") << "\n";
    ...
    nAnonymizeAdvantageAmount = GetArg("-anonymizeadvantageamount", 0);
    if (nAnonymizeAdvantageAmount > 999999) nAnonymizeAdvantageAmount = 999999;
    if (nAnonymizeAdvantageAmount < 2) nAnonymizeAdvantageAmount = 2;
    ...
    LogPrintf("Anonymize Advantage Amount %d\n", nAnonymizeAdvantageAmount);
    ...
    /* Denominations
       ...
       For example:
       1USDA+1000 == (.1USDA+100)*10
       10USDA+10000 == (1USDA+1000)*10
    */
    obfuScationDenominations.push_back((10000 * CREDIT) + 10000000);
    obfuScationDenominations.push_back((1000 * CREDIT) + 1000000);
    obfuScationDenominations.push_back((100 * CREDIT) + 100000);
    obfuScationDenominations.push_back((10 * CREDIT) + 10000);
    obfuScationDenominations.push_back((1 * CREDIT) + 1000);
    obfuScationDenominations.push_back((.1 * CREDIT) + 100);
    /* Disabled till we need them
    obfuScationDenominations.push_back( (.01      * CREDIT)+10 );
    obfuScationDenominations.push_back( (.001     * CREDIT)+1 );
    */
...
#ifdef ENABLE_WALLET
    // Generate credits in the background
    if (pwalletMain)
