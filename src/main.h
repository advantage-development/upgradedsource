...
#ifndef BITCREDIT_MAIN_H
#define BITCREDIT_MAIN_H
...
static const unsigned int DEFAULT_BLOCK_MAX_SIZE = 500000000; //50MB
...
static const unsigned int MAX_STANDARD_TX_SIZE = 250000000;
...
static const int CREDITBASE_MATURITY = 1;
...
extern CScript CREDITBASE_FLAGS;
...
bool InitBlockIndex();
/** Load the block tree and credits database from disk */
bool LoadBlockIndex();
...
{
public:
    std::vector<CTxUndo> vtxundo; // for all but the creditbase
...
/** Undo the effects of this block (with given index) on the UTXO set represented by credits.
 *  In case pfClean is provided, operation will try to be tolerant about errors, and *pfClean
 *  will be true if no problems were found. Otherwise, the return value will be false in case
 *  of problems. Note that in any case, credits may be modified. */
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& credits, bool* pfClean = NULL);

/** Reprocess a number of blocks to try and get on the correct chain again **/
bool DisconnectBlocksAndReprocess(int blocks);

/** Apply the effects of this block (with given index) on the UTXO set represented by credits */
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& credits, bool fJustCheck = false);
...
/** RAII wrapper for VerifyDB: Verify consistency of the block and credit databases */
class CVerifyDB
{
public:
    CVerifyDB();
    ~CVerifyDB();
    bool VerifyDB(CCoinsView* creditsview, int nCheckLevel, int nCheckDepth);
};
...
extern CCoinsViewCache* pcreditsTip;
